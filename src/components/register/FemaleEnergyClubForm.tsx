...
export default function FemaleEnergyClubForm({ isOpen = true }: Props) {
      ...
      try {
        ...
        try {
          console.log(`[doUploadsAndPatch] Phase 1: Seeding file paths`, {
            authBefore: auth.currentUser ? 'authenticated' : 'not authenticated',
            cvPath,
            designPath,
          ...
          if (!auth.currentUser) {
            ...
            console.log(`[doUploadsAndPatch] Authenticated anonymously for path seeding`, {
              authAfter: auth.currentUser ? 'authenticated' : 'not authenticated'
            });
          }
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const { cvFile, designFile, ...rest } = values as unknown as ApplicationFemaleEnergyClub;
      const applicationId = await createApplication(rest as unknown as ApplicationFemaleEnergyClub);
  ...
  const submitFromSummary = async () => {
    try {
      await onSubmit(getValues() as FormValues);
    } catch {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 flex items-center justify-center px-4 py-8">
        <div className="mx-auto max-w-2xl w-full">
          {/* Success Animation */}
          <div className="text-center mb-8 animate-fade-in-up">
            <div className="relative inline-flex items-center justify-center w-24 h-24 bg-pink-100 rounded-full mb-6 animate-bounce-custom">
              <div className="absolute inset-0 bg-pink-400 rounded-full animate-ping opacity-75"></div>
              <CheckCircle2 className="relative h-12 w-12 text-pink-600" />
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
              🌟 Application Submitted!
            </h1>
            <p className="text-xl text-gray-600">
              Welcome to the Female Energy Club community
            </p>
          </div>
            ...
            <CardContent className="p-8">
              <div className="space-y-6">
                ...
                <div className="text-center space-y-3">
                  ...
                  <p className="text-gray-700 leading-relaxed">
                    Your Female Energy Club application has been received and is now under review.
                    You're joining a supportive network of women leaders in energy.
                  </p>
                </div>
                ...
                <div className="bg-pink-50 border border-pink-200 rounded-lg p-4">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      <p className="text-sm font-medium text-pink-900">Application Reference ID</p>
                      <p className="text-pink-700 font-mono text-lg">{successId}</p>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">Welcome to FEC</p>
                        <p className="text-gray-600 text-sm">If accepted, you'll receive onboarding information and mentorship program details.</p>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  const teamOptions = leadershipInterest
    ? [...FEC_TEAMS_WITH_ECE]
    : [...FEC_TEAMS_WITH_ECE].filter((t) => t !== 'Energy Community Engagement ECE');
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      {showPreview ? (
        ...
        <FormSummary
          ...
          programTitle="Female Energy Club"
        />
      ) : (
        // Form Input Mode
